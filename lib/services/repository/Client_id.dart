...
class ClientService {
  ...
  Future<Resource<List<ClientId>>> getClient(String token, int clientID) async {
    ...
    var request = http.Request(
      'GET',
      Uri.parse(
        '${AppUtils.baseUrl}/Client/GetClientListByID?clientID=$clientID',
      ),
    );
    ...
    if (response.statusCode == 200) {
      ...
      return Resource.success(
        jsonData.map((item) => ClientId.fromJson(item)).toList(),
      );
    } else {
      debugPrint('Response status: ${response.statusCode}');

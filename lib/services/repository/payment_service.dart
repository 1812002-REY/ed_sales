...
class PaymentModesService {
  ...
  Future<Resource<List<PaymentModes>>> getAllPayments(String token) async {
    ...
    var request = http.Request(
      'GET',
      Uri.parse('${AppUtils.baseUrl}/Lookup/GetPaymentMethods'),
    );
    ...
    if (response.statusCode == 200) {
      ...
      return Resource.success(
        jsonData.map((item) => PaymentModes.fromJson(item)).toList(),
      );
    } else {
      debugPrint('Response status: ${response.statusCode}');

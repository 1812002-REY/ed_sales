...
class StaffService {
  ...
  Future<Resource<List<STaffs>>> getStaff(String token) async {
    ...
    var request = http.Request(
      'GET',
      Uri.parse('${AppUtils.baseUrl}/Staff/GetStaffList'),
    );
    ...
    if (response.statusCode == 200) {
      ...
      return Resource.success(jsonData.map((item) => STaffs.fromJson(item)).toList());
    } else {
      debugPrint('Response status: ${response.statusCode}');
      debugPrint('Response status: ${response.reasonPhrase}');
      return Resource.error('Failed to load staff');
    }
  }
}
